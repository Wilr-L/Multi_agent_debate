...
# ─────────────────────────────────────────────
# 3. VLM Interface — SiliconFlow Chat Completions
# ─────────────────────────────────────────────
#
# Client for SiliconFlow's OpenAI-compatible chat-completions endpoint.
#   Endpoint:  POST {base_url}/chat/completions
#   Auth:      Bearer token via constructor `api_key=` or $SILICONFLOW_API_KEY
#   Default model: Qwen/Qwen3-VL-32B-Instruct  (any SF-hosted VLM works)
...
class VLMInterface:
    """SiliconFlow Chat-Completions client (Qwen3-VL-32B-Instruct by default)."""
    ...
    def query(self, user_prompt: str, image_path: Optional[str] = None) -> str:
        ...
        if not self.api_key:
            raise RuntimeError(
                "SiliconFlow API key not set. Pass `api_key=` to VLMInterface "
                "or export SILICONFLOW_API_KEY in the environment."
            )
        ...
        # ── Outer loop: retry the whole completion if SiliconFlow returns
        ...
        for attempt in range(max_empty_retries):
            ...
            # SiliconFlow / Qwen3 thinking-mode models put the user-visible
            ...
            try:
                ...
                raise RuntimeError(
                    f"Unexpected response shape from SiliconFlow: {data!r}"
                ) from e
            ...
            raise RuntimeError(
                f"SiliconFlow returned empty content AND empty reasoning_content. "
    ...
    def _encode_image_as_data_url(image_ref: str) -> str:
        ...
        HTTPS / data: URLs are returned unchanged (SiliconFlow accepts both).
        """
    ...
    def _post_with_retry(self, path: str, payload: dict) -> dict:
        ...
        raise RuntimeError(
            f"SiliconFlow API failed after {self.max_retries} attempts. Last error: {last_err}"
        )
...
def main():
    ...
    if not os.environ.get("SILICONFLOW_API_KEY"):
        ...
        print("        On PowerShell:  $env:SILICONFLOW_API_KEY = 'sk-...'")
        return
