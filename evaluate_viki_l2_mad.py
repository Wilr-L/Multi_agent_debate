...
Adaptation to VIKI-L2 plans, faithful to the paper's 3-role design +
fallback judge:
...
Key differences from existing multi-agent baselines
---------------------------------------------------
  vs main debate / VD : main uses "two consecutive ACCEPTs" as the stop
                       rule, MAD uses a THIRD moderator agent + explicit
                       adversarial framing ("You disagree with my answer").
  vs Proposer-Critic : PC is asymmetric (VLM1 = proposer, VLM2 = critic).
                       MAD is symmetric between debaters, with a THIRD
                       role (moderator) that decides consensus.
  vs Self-Refine     : MAD is 3-agent debate + judge, SR is 1-agent
                       self-critique.

Faithful to paper: NO retry loop. `debate_loops = 1` in results.json.
...
python evaluate_viki_l2_mad.py \
    --model-path /path/to/Qwen2.5-VL-32B-Instruct \
    --tensor-parallel-size 2 --max-model-len 8192 --gpu-mem-util 0.85 \
    --enforce-eager --max-num-seqs 4 \
    --max-rounds 3 --limit 400 --trust-remote-code
"""
...
# Round-1 opening for the affirmative — just present the task.
...
# Round-1 negative — sees affirmative's plan and MUST DISAGREE.
...
# Rebuttal prompt used from round 2 onwards. Each side is shown the
...
def _render_robot_block(robots: dict) -> str:
    ...
    relevant = set()
    for rname in robots.values():
        if rname:
            relevant.update(AGENT_AVAIL_ACTIONS.get(rname, []))
    action_block = "\n".join(f"- {ACTION_DESCRIPTION[a]}"
                             for a in ACTION_DESCRIPTION if a in relevant)
    r1_name = robots.get("R1") or ""
    r2_name = robots.get("R2") or ""
    return _MAD_ROBOT_BLOCK_TEMPLATE.format(
        r1_id="R1", r1_name=r1_name,
        r1_description=ROBOT_DESCRIPTION.get(r1_name, ""),
        r1_avail_actions=AGENT_AVAIL_ACTIONS.get(r1_name, []),
        r2_id="R2", r2_name=r2_name,
        r2_description=ROBOT_DESCRIPTION.get(r2_name, ""),
        r2_avail_actions=AGENT_AVAIL_ACTIONS.get(r2_name, []),
...
def run_one_task(task, llm, sampling_params, args, logger, stats):
    ...
    # 3 roles share one LLM instance; each gets its own conversation-history
    # bookkeeping (which is stateless-sent to vLLM per query, but retained
    # for logging).
...
def parse_args():
    p = argparse.ArgumentParser(
        description="Multi-Agents Debate (MAD, Liang et al. EMNLP 2024) on VIKI-L2")
    add_common_args(p)
